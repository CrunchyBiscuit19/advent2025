...
public:
    ...
    void unionSet(Coord& i, Coord& j) {
        ...
        auto iClone = Coord(i.x, i.y, i.z);               
        auto jClone = Coord(j.x, j.y, j.z);               
        auto& x = findSet(iClone);
        auto& y = findSet(jClone);  
        ...
        if (setSize.at(higherRankNode) == NUM_POINTS) std::println("{} x {} = {}", i.x, j.x, i.x * j.x);
        numSets--;
    }
};
...
int main(int argc, char *argv[]) {
    ...
    UnionFind uf;
    for (long i  = 0; i < sqDistances.size(); i++) uf.unionSet(sqDistances[i].from, sqDistances[i].to);
